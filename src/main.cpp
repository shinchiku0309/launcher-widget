...
static void LoadConfig() {
    ...
    g.config.buttonSize = GetPrivateProfileIntW(L"Window", L"ButtonSize", 78, g.configPath.c_str());
    g.config.gap = GetPrivateProfileIntW(L"Window", L"Gap", 8, g.configPath.c_str());
...
static LRESULT CALLBACK ButtonEditorProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_CREATE: {
        ...
        AddLabel(hwnd, L"Title", 14, 16, 78, 22);
        AddEdit(hwnd, IDC_TITLE, ctx->original.title, 96, 14, 330, 24);
        AddLabel(hwnd, L"Text", 14, 50, 78, 22);
        AddEdit(hwnd, IDC_TEXT, ctx->original.text, 96, 48, 330, 24);
        ...
        SendMessageW(combo, CB_SELECTSTRING, static_cast<WPARAM>(-1), reinterpret_cast<LPARAM>(ActionToString(ctx->original.action.type).c_str()));
        AddLabel(hwnd, L"Target", 14, 152, 78, 22);
        AddEdit(hwnd, IDC_TARGET, ctx->original.action.target, 96, 150, 330, 24);
    ...
    case WM_COMMAND:
        ...
        if (LOWORD(wp) == IDOK && ctx) {
            ...
            wchar_t actionName[64]{};
            HWND combo = GetDlgItem(hwnd, IDC_ACTION);
            int sel = static_cast<int>(SendMessageW(combo, CB_GETCURSEL, 0, 0));
            if (sel >= 0) SendMessageW(combo, CB_GETLBTEXT, sel, reinterpret_cast<LPARAM>(actionName));
            ctx->target->action.type = ActionFromString(actionName);
...
static void EditButton(int index) {
    ...
    HWND dialog = CreateWindowExW(WS_EX_DLGMODALFRAME, L"LauncherButtonEditor", L"Edit Button",
        WS_CAPTION | WS_SYSMENU, CW_USEDEFAULT, CW_USEDEFAULT, 460, 310,
...
static LRESULT CALLBACK SettingsProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_CREATE: {
        ...
        SetWindowLongPtrW(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(ctx));
        AddLabel(hwnd, L"Rows", 14, 18, 100, 22);
        AddEdit(hwnd, IDC_ROWS, std::to_wstring(ctx->original.rows), 128, 16, 120, 24);
        AddLabel(hwnd, L"Columns", 14, 52, 100, 22);
        AddEdit(hwnd, IDC_COLS, std::to_wstring(ctx->original.cols), 128, 50, 120, 24);
        AddLabel(hwnd, L"Button size", 14, 86, 100, 22);
        AddEdit(hwnd, IDC_BUTTON_SIZE, std::to_wstring(ctx->original.buttonSize), 128, 84, 120, 24);
    ...
    case WM_COMMAND:
        if (LOWORD(wp) == IDOK && ctx) {
            ...
            g.config.buttonSize = std::max(48, std::min(160, _wtoi(GetWindowTextString(GetDlgItem(hwnd, IDC_BUTTON_SIZE)).c_str())));
            g.config.gap = std::max(2, std::min(24, _wtoi(GetWindowTextString(GetDlgItem(hwnd, IDC_GAP)).c_str())));
...
static void ShowSettingsDialog() {
    ...
    HWND dialog = CreateWindowExW(WS_EX_DLGMODALFRAME, L"LauncherSettingsEditor", L"Settings",
        WS_CAPTION | WS_SYSMENU, CW_USEDEFAULT, CW_USEDEFAULT, 350, 280,
        g.hwnd, nullptr, g.instance, &ctx);
    RunOwnedModal(dialog);
    if (ctx.accepted) {
        CurrentButtons();
